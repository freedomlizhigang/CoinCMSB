...
class TagService
{
    ...
    * 取推荐文章，不带分页
    ...
    public function push($cid = 0,$num = 5)
    {
        try {
            $cid = explode(',', $cid);
            $push = Article::select('id','cate_id','title','keywords','describe','url','thumb','source','publish_at','push_flag')->whereIn('cate_id',$cid)->where('del_flag',0)->where('push_flag',1)->where('publish_at','<=',date('Y-m-d H:i:s'))->where('status',99)->limit($num)->orderBy('sort','desc')->orderBy('id','desc')->get();
            return $push;
        } catch (\Throwable $e) {
            return [];
        }
    ...
    * 取文章，不带分页
    ...
    public function list($cid = 0,$num = 5)
    {
        try {
            $cid = explode(',', $cid);
            $list = Article::select('id','cate_id','title','keywords','describe','url','thumb','source','publish_at','push_flag')->whereIn('cate_id',$cid)->where('del_flag',0)->where('publish_at','<=',date('Y-m-d H:i:s'))->where('status',99)->limit($num)->orderBy('push_flag','desc')->orderBy('sort','desc')->orderBy('id','desc')->get();
            return $list;
        } catch (\Throwable $e) {
            return [];
        }
